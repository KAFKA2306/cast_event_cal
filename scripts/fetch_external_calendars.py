...
VRC_SEARCH_START_RE = re.compile(
    r'(?:開始|Starts?|Empieza|Beginnt|Начало|Commence)\s*(?P<value>\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2})',
    flags=re.IGNORECASE,
)
VRC_SEARCH_END_RE = re.compile(
    r'(?:終了|Ends?|Termina|Endet|Окончание|Se termine)\s*(?P<value>\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2})',
    flags=re.IGNORECASE,
)
