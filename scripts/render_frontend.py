...
function assetProofHtml(e){const rows=[];const links=Array.isArray(e.official_links)?e.official_links:[];const group=links.find(r=>String(r?.kind||'')==='vrchat_group'&&String(r?.url||'').startsWith('https://'));const groupUrl=String(group?.url||e.vrchat_group_url||'');if(groupUrl.startsWith('https://'))rows.push(`<a href="${esc(groupUrl)}" target="_blank" rel="noopener noreferrer">VRChat Group</a>`);if(String(e.official_x_url||'').startsWith('https://'))rows.push(`<a href="${esc(e.official_x_url)}" target="_blank" rel="noopener noreferrer">公式X</a>`);if(String(e.official_website_url||'').startsWith('https://'))rows.push(`<a href="${esc(e.official_website_url)}" target="_blank" rel="noopener noreferrer">公式Web</a>`);const imageUrl=String(e.preferred_image_url||e.vrchat_group_image_url||e.image_url||'');if(imageUrl.includes('webp'))rows.push(`<a href="${esc(imageUrl)}" target="_blank" rel="noopener noreferrer">WebP画像</a>`);return rows.length?`<div class="asset-proof">${rows.join('')}</div>`:''}
""".strip()
