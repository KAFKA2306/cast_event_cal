...
def main(argv: list[str] | None = None) -> int:
    ...
    command_file = Path(command[1]) if len(command) > 1 and Path(command[0]).name.startswith("python") and command[1].endswith(".py") else None
    input_specs = list(args.input)
    if command_file is not None and command_file.exists():
        input_specs.append(command_file.as_posix())
    ...
    finished_at = utc_now()
    duration = max(0.0, time.perf_counter() - started_counter)
    ...
    append_record(report_path, record)
