...
def test_fingerprint_is_stable_and_ignores_runtime_cache(tmp_path: Path) -> None:
    ...
    cache.mkdir()
    (cache / "module.pyc").write_bytes(b"runtime")
    assert run_stage.fingerprint_paths(tmp_path, [str(package)]) == stable


def test_stage_records_output_change_and_propagates_failure(
    ...
    create = [
        ...
        "--output", "output.txt", "--", sys.executable, "-c",
        "from pathlib import Path; Path('output.txt').write_text('created')",
    ]
    assert run_stage.main(create) == 0
    ...
    assert run_stage.main(fail) == 7
    assert json.loads(report.read_text().splitlines()[-1])["status"] == "failed"


def test_finalize_reports_git_churn_and_network_costs(
    tmp_path: Path, monkeypatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    subprocess.run(
        ["git", "commit", "-m", "init"], cwd=root, check=True, stdout=subprocess.PIPE
    )
    ...
    records = [
        {
            ...
            "measurements_after": {
                "health.json": {
                    "reported_request_count": 22,
                    "reported_fetched_records": 350,
                }
            },
        }
    ]
