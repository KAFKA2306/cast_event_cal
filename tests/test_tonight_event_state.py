...
def _state_for(event: dict[str, str], now: str) -> str:
    ...
    instrumented = source.replace(marker, "globalThis.__tonightStateFor=stateFor})();", 1)
    script = instrumented + "\nprocess.stdout.write(globalThis.__tonightStateFor(" + json.dumps(event) + ",new Date(" + json.dumps(now) + ")));"
    result = subprocess.run([node, "-e", script], check=True, capture_output=True, text=True)
    return result.stdout


def test_tonight_state_distinguishes_ended_from_active_and_future() -> None:
    now = "2026-09-14T12:00:00+09:00"
    assert _state_for({}, now) == "unknown"
    assert _state_for({"starts_at": "2026-09-14T09:00:00+09:00", "ends_at": "2026-09-14T10:00:00+09:00"}, now) == "ended"
    assert _state_for({"starts_at": "2026-09-13T23:30:00+09:00", "ends_at": "2026-09-14T12:30:00+09:00"}, now) == "live"
